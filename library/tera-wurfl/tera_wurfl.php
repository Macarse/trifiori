 ...
 * @version Stable 1.5.1 $Date: 2007/05/09 20:09:13 $
 * @license http://www.mozilla.org/MPL/ MPL Vesion 1.1
 * $Id: tera_wurfl.php,v 1.1.4.6.2.20 2007/05/09 20:09:13 kamermans Exp $
...
*/
		...
		if(count($_short_wurfl_ua) > 0){
			...
			// no match was found
			$this->_AddUAToCache($this->user_agent,$this->capabilities,$this->device_root);
			$this->_toLog('getDeviceCapabilitiesFromAgent', 'No match was found in DB', LOG_INFO);
			return(false);
		}
	}
	/**
	...
	function _UserAgentInCache($ua){
		//TODO: using LIKE will do the search case-insensitive, but = is much faster
		$query = "SELECT * FROM ".DB_CACHE_TABLE." WHERE user_agent = ".$this->_sqlPrep($ua);
		$res = mysql_query($query,$this->dbcon);
		$this->num_queries++;
		$cache_data = mysql_fetch_assoc($res);
		$ret = (mysql_num_rows($res) > 0)? $cache_data['cache_data']: false;
//if(!$ret)die($query);
		return($ret);
	}
	/**
	...
	function _AddUAToCache($ua,$cap,$devroot){
		...
		$cache_data = serialize(array("capabilities"=>$cap,"device_root"=>$devroot));
		// FIXME: DELAYED INSERTS only work on MyIASM
		mysql_query("INSERT DELAYED INTO ".DB_CACHE_TABLE." (user_agent,cache_data) VALUES (".$this->_sqlPrep($ua).",".$this->_sqlPrep($cache_data).")",$this->dbcon);
		$this->_toLog('_AddUAToCache', 'Added UA to cache: '.$ua, LOG_INFO);
		$this->num_queries++;
		return(true);
	}
	/**
	...
	/**
	 * Given a string, will escape any unfriendly characters and return
