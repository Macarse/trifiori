 ...
 * $Id: tera_wurfl_parser.php,v 1.1.2.3.2.16 2007/05/09 20:09:13 kamermans Exp $
...
function clear_cache(){
	mysql_query("DROP TABLE IF EXISTS ".DB_CACHE_TABLE);
	$createtable = "CREATE TABLE `".DB_CACHE_TABLE."` (
  `user_agent` varchar(255) binary NOT NULL default '',
  `cache_data` mediumtext NOT NULL,
  PRIMARY KEY  (`user_agent`)
) TYPE=".DB_TYPE;
	mysql_query($createtable) or die("ERROR: could not create cache table (".DB_CACHE_TABLE."): ".mysql_error());
	return(true);
}
	...
	while($new = mysql_fetch_assoc($patchres)){
		...
		foreach($newcap as $key => $val) {
			if ( is_array($val) ) {
				...
				// Thanks to Mait Vilbiks for noticing that PHP5 requires 2 arrays for array_merge()
				// changed by kamermans 19Feb2007 (tentative release 1.5)
				$mergedcap[$key] = @array_merge((array)$mergedcap[$key], $val);
			} else {
				$mergedcap[$key] = $val;
			}
		}
