...
class Zend_Paginator_Adapter_DbSelect implements Zend_Paginator_Adapter_Interface
{
...

  {
      ...
      if ($this->_rowCount === null) {
          ...
          foreach($this->_select->getPart(Zend_Db_Select::FROM) as $name => $table)

          {
...

              {
                  ...
                  $rowCount->from(array($name => $table['tableName']));

              }

              elseif($table['joinType'] == 'inner join')        

              {

                  $rowCount->join(array($name => $table['tableName']), $table['joinCondition']);
